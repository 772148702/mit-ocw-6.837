...
Grid* SceneParser::parseGrid() {
	...
	// read in the left-bottom corner of the grid
	getToken(token); assert(!strcmp(token, "min"));
	Vec3f min = readVec3f();

	// read in the right-upper corner of the grid
	getToken(token); assert(!strcmp(token, "max"));
	Vec3f max = readVec3f();

	// read in the num of cells of the grid
	getToken(token); assert(!strcmp(token, "numCells"));
	int nx = readInt();
	int ny = readInt();
	int nz = readInt();

	Grid *answer = new Grid(min, max, nx, ny, nz, materials[current_material_index]);

	// read in the rasterizeSphere 
...
Object3D* SceneParser::parseObject(char token[MAX_PARSER_TOKEN_LENGTH]) {
  ...
  if (!strcmp(token, "MaterialIndex")) {
	current_material_index = readInt();	
	getToken(token);
  }
...
void SceneParser::parseMaterials() {
  ...
  while (num_materials > count) {
    ...
    if (!strcmp(token, "PhongMaterial")) {
	  Vec3f diffuseColor(0.0f, 0.0f, 0.0f), 
		  specularColor(0.0f, 0.0f, 0.0f), 
		  reflectiveColor(0.0f, 0.0f, 0.0f), 
		  transparentColor(0.0f, 0.0f, 0.0f);
	  float exponent = 0.0f, indexOfRefraction = 0.0f;
      ...
      materials[count] = (Material *) new PhongMaterial(diffuseColor, specularColor, exponent, transparentColor, reflectiveColor, indexOfRefraction);
      // ++++++++++++++++++++++++++++++++++++++++++++++++++++
...
Sphere* SceneParser::parseSphere() {
  ...
  return new Sphere(center,radius,materials[current_material_index]);
}
...
Box* SceneParser::parseBox() {
  ...
  return new Box(min,max,materials[current_material_index]);
}
...
Plane* SceneParser::parsePlane() {
  ...
  return new Plane(normal,offset,materials[current_material_index]);
  // ++++++++++++++++++++++++++++++++++++++++++++++++++++
}
...
Triangle* SceneParser::parseTriangle() {
  ...
  return new Triangle(v0,v1,v2,materials[current_material_index]);
  // ++++++++++++++++++++++++++++++++++++++++++++++++++++
}
  ...
  while (1) {
    ...
    if (c == EOF) { break;
      ...
      Triangle *t = new Triangle(verts[f0-1],verts[f1-1],verts[f2-1],
                               materials[current_material_index]);
      // ++++++++++++++++++++++++++++++++++++++++++++++++++++
